...
// Tab functionality for modals with tabs
const initModalTabs = () => {
  const modalTabs = document.querySelectorAll('.modal-tab');
  
  // Show GitHub button if GitHub content exists
  const modalsWithTabs = document.querySelectorAll('.modal-content--with-tabs');
  modalsWithTabs.forEach((modalContent) => {
    const modal = modalContent.closest('.project-modal');
    if (!modal) return;
    
    const githubContent = modal.querySelector('.modal-tab-content[data-content="github"]');
    const githubTab = modal.querySelector('.modal-tab[data-tab="github"]');
    
    if (githubContent && githubTab) {
      githubTab.style.display = 'grid';
    }
  });
  ...
  modalTabs.forEach((tab) => {
    tab.addEventListener('click', (e) => {
      ...
      const modal = tab.closest('.project-modal');
      if (!modal) return;
      
      // Remove active class from all tabs and contents
      const allTabs = modal.querySelectorAll('.modal-tab');
      const allContents = modal.querySelectorAll('.modal-tab-content');
      ...
      const content = modal.querySelector(`.modal-tab-content[data-content="${tabName}"]`);
      if (content) {
        content.classList.add('active');
      }
      
      // Special handling for download tab
      if (tabName === 'download') {
        const downloadLink = modal.querySelector('.modal-download-action a[download]');
        if (downloadLink) {
          // Auto-trigger download after a short delay for better UX
          setTimeout(() => {
            downloadLink.click();
            // Switch back to info tab after download
            setTimeout(() => {
              const infoTab = modal.querySelector('.modal-tab[data-tab="info"]');
              if (infoTab) {
                infoTab.click();
              }
            }, 500);
      ...
      // Special handling for GitHub tab - open in new tab
      if (tabName === 'github') {
        const githubLink = modal.querySelector('.modal-download-action a[href*="github.com"]');
        if (githubLink) {
          // Open GitHub link in new tab
          setTimeout(() => {
            window.open(githubLink.href, '_blank', 'noopener,noreferrer');
            // Switch back to info tab after opening
            setTimeout(() => {
              const infoTab = modal.querySelector('.modal-tab[data-tab="info"]');
              if (infoTab) {
                infoTab.click();
